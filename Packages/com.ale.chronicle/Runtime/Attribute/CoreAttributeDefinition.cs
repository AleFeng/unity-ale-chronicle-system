...
{
    /// <summary>
    /// 核心属性的定义单元（力量/敏捷/智力/魅力/体力/统帅…）：各子系统「加成的靶子」。
    /// 值 = 「基础值 + 修饰器栈」，当前值由 <see cref="CoreAttributeResolver"/> 现场求出。
    /// <para>面向玩家的文本 / 资源字段用 <see cref="AttributeValue"/>（Text 走本地化、Sprite 走 Addressable 统一收集）；
    /// <see cref="id"/> 与 <see cref="categoryEnumRef"/> 是稳定键，保持裸 <see cref="string"/>。</para>
    /// </summary>
    [Serializable]
    public class CoreAttributeDefinition
    {
        ...
        {
            return new CoreAttributeDefinition
            {
                id              = id,

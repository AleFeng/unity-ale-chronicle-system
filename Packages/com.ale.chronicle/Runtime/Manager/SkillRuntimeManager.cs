...
namespace Ale.Chronicle
{
    ...
    /// 提供学习 / 遗忘 / 查询 / 存档接口。技能目录来自已注册数据库（经 <see cref="ChronicleDataManager"/> 查询）。</para>
    ///
    /// <para>继承 toolkit 的 <see cref="ToolkitSingleton{T}"/>，关闭 Domain Reload 时也能在每次播放开始复位静态状态
    /// （不串数据）。已学技能变化时触发 <see cref="OnLearnedChanged"/> 供技能 UI 刷新（UI 层 S3 / 后续接入）。</para>
    /// </summary>

...
namespace Ale.Chronicle
{
    ...
    /// 编辑器得以复用同一套绘制（<c>SkillConfigDrawer</c>，S2 移植）；从模板创建技能时复制这些默认值到新技能。
    ///
    ...
    public interface ISkillConfig
    {
        ...
        /// <summary>主分组标签 ID（单选，引用 <see cref="ChronicleDatabase.GroupTags"/> 的 id）。</summary>
        string PrimaryGroupTag { get; set; }

        /// <summary>副分组标签 ID 列表（可多选，引用 <see cref="ChronicleDatabase.GroupTags"/> 的 id）。</summary>

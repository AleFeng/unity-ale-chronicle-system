...
namespace Ale.Chronicle
{
    ...
    /// 技能是配置目录；最终由角色使用、主要影响角色（效果整合见后置 S3）。
    ///
    ...
    {
        /// <summary>唯一标识（在技能列表中做重复检查；角色 / 其它系统通过此 ID 引用技能）。</summary>
        public string id;
        ...
        /// <summary>
        /// 技能图标（<see cref="EFieldType.Sprite"/> 对象类属性值：直接引用或 Addressable 授权 GUID）。
        ...
        /// <summary>主分组标签 ID（单选，引用统一分组标签池 <see cref="ChronicleDatabase.GroupTags"/> 的 id）。</summary>
        ...
        public void RebuildAttributes(IChronicleSchemaSource src)
        {
            ...
            var template = src.GetSkillTemplate(templateRef);

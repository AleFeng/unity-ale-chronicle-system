...
> ⚠️ **当前版本 `0.2.0`**：角色 / 属性 / 特质 / 技能 / 职业 / 头衔 六大领域的配置与运行时数据基础已可用；「百万级人生模拟」中的世代推进、角色随机生成、继承结算、派生属性等尚为**预留**、暂未接入。
...
3. **模板 + 实例** —— 角色 / 属性 / 特质 / 技能 每类有「模板（蓝图）+ 定义 / 实例」两层，Schema 自动对齐、深拷贝复用；职业 / 头衔 为定长强类型定义，转职树 / 阶级序列 为组织它们的结构对象。
4. **统一合流 + 条件** —— 核心属性走「基础值 + 修正器合流」（来源含特质 / 职业每级成长 / 头衔加成，带逐来源拆解）；特质 / 职业 / 头衔的「获得 / 从业条件」接入 `Ale.Condition` 条件系统。
...
| 职业系统 | 等级上限 / 经验曲线（公式·表格·曲线）/ 每级成长汇入核心属性 / 等级解锁 / 转职树（进阶 DAG，编辑器加子防环）；运行时 `AddExp` 按曲线升级、封顶、施加解锁。 |
...
| 单向导出 | `ChronicleConfigSerializer` → 紧凑二进制（魔数 `CHRO`，格式 v4），**append-only 向后兼容**旧版本；对象引用以 AssetGUID 承载。 |
...
| **职业系统** | 职业（等级上限 / 经验曲线 / 每级成长 / 解锁 / 从业条件）、转职树 | `ProfessionRuntimeManager`（AddExp 按曲线升级 + 解锁）；成长汇入核心属性 |
...
```
https://github.com/AleFeng/unity-ale-chronicle-system.git?path=/Packages/com.ale.chronicle#0.2.0
```
...
工程内演示与测试：`Assets/Demo/`（技能 UI）、`Assets/DemoInventory/`（Chronicle × Inventory 整合）、`Assets/Editor/DemoWizard/`（预制体生成）、`Assets/Tests/`（18 个 EditMode 测试，含职业 / 头衔）。
